...
struct GenomeHit {
    ...
    bool compatibleWith(
                        ...
                        index_t intronLen_thresh = maxIntronLen) const;
...
bool GenomeHit<index_t>::compatibleWith(
                                        ...
                                        index_t intronLen_thresh) const
{
    ...
    if(rddif != refdif) {
        if(rddif > refdif) {
            ...
            if(refdif - rddif < minIntronLen) {
                ...
                if(refdif - rddif > intronLen_thresh) {
                    return false;
                }
            }
        }
...
{
    if(this == &otherHit) return false;
    assert(compatibleWith(otherHit, no_spliced_alignment));
    ...
    if(!spliced && !ins && !del && this_rdoff + this_len == other_rdoff) {
        ...
        _len += otherHit._len;
        _score = calculateScore(rd, ssdb, sc, minK_local, ref);
        assert(repOk(rd, ref));
        return true;
    }
        ...
        if(spliced && spliceSite == NULL) {
            ...
            if(maxspldir == EDIT_SPL_UNKNOWN) {
                if(shorter_anchor_len < noncan_mal) {
                    float intronLenProb = intronLen_prob_noncan(shorter_anchor_len, other_toff - this_toff);
                    if(intronLenProb > 0.01f)
                        return false;
                }
            } else {
                if(shorter_anchor_len < can_mal) {
                    float intronLenProb = intronLen_prob(shorter_anchor_len, other_toff - this_toff);
                    if(intronLenProb > 0.01f)
                        return false;
                }
            }
